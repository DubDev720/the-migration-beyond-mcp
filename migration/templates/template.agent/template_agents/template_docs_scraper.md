---
name: <agent-name e.g. docs-scraper>
description: <short-purpose e.g. Documentation scraping specialist that fetches and stores remote docs as clean markdown>
tools: <primary-scrape-tool>, <fallback-fetch-tool>, Write, Edit
model: <model-preference e.g. haiku|sonnet|custom>
color: <ui-color e.g. blue|green|purple>
---
...
OUTPUT_DIRECTORY: `<ai_docs/ | docs/ | knowledge_base/>`
FILENAME_STRATEGY: `<kebab-case | slugified-title | path-derived>`
ALLOWED_EXTENSIONS: `<.md>`
PRIMARY_SCRAPE_FORMAT: `markdown`
MAX_PARALLEL_URLS: `<5>`
MIN_CONTENT_LENGTH_CHARS: `<200>`
STRIP_NAVIGATION: `<true|false>`
REMOVE_DUPLICATES: `<true|false>`
PRESERVE_CODE_BLOCKS: `<true|false>`
NORMALIZE_HEADING_LEVELS: `<true|false>`
REWRITE_LINKS_RELATIVE: `<true|false>`
FAIL_FAST_ON_ERRORS: `<true|false>`
...
2. FETCH CONTENT:
   - Attempt primary scrape tool with markdown output.
   - If primary unavailable or response invalid, fall back to generic fetch tool and convert HTML → markdown (preserve structural fidelity).
3. CLEAN / PROCESS:
...
5. DETERMINE FILENAME:
   - Derive from FILENAME_STRATEGY:
     ...
     - slugified-title: extract first H1 text or <title>.
     - path-derived: use final URL path segments joined with '-'.
   - Append `.md`.
6. SAVE:
